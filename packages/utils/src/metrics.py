...
class RecoveryMetricsTracker:
    ...
    @staticmethod
    def calculate_mttd(
        detection_times: Sequence[datetime], fault_times: Sequence[datetime]
    ) -> float:
        """
        ...
        deltas = [
            (d - f).total_seconds()
            for d, f in zip(detection_times, fault_times, strict=False)
            if d > f
        ]
        if not deltas:
            return 0.0
    ...
    @staticmethod
    def calculate_mttr(
        recovery_times: Sequence[datetime], detection_times: Sequence[datetime]
    ) -> float:
        """
        ...
        deltas = [
            (r - d).total_seconds()
            for r, d in zip(recovery_times, detection_times, strict=False)
            if r > d
        ]
        if not deltas:
            return 0.0
