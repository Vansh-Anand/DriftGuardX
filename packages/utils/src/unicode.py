...
def secure_normalize(text: str, max_decode_passes: int = 3, max_length: int = 500_000) -> str:
    ...
    if len(text) > max_length:
        raise UnicodeNormalizationError(
            f"Text length ({len(text)}) exceeds maximum allowed ({max_length})."
        )
    ...
    if re.search(r"[\u202A-\u202E\u2066-\u2069]", normalized):
        raise UnicodeNormalizationError(
            "BiDi override characters detected. Possible payload obfuscation."
        )
...
def aggressive_normalize_for_banlist(
    text: str, max_decode_passes: int = 3, max_length: int = 500_000
) -> str:
    """
