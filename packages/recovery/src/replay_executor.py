                ...
                if span.get("component_type") == action.target_component:
                    result_spans[i] = {
                        **span,
                        "output": {
                            "status": "recovered",
                            "action": getattr(action, "action_type", None),
                        },
                        "_intervened": True,
                    }
...
class RecoveryActionApplier:
    ...
    def apply(self, action: Any, pipeline: Any) -> bool:
        ...
        if action_type in (
            RecoveryActionType.REPLACE,
            RecoveryActionType.ROLLBACK,
            RecoveryActionType.RECONFIGURE,
        ):
...
class LocalPipelineRecoveryReplayExecutor(RecoveryReplayExecutor):
    ...
    def __init__(
        self, pipeline_factory: Callable[[], Any], applier: RecoveryActionApplier | None = None
    ):
    ...
    def replay(
        ...
        def _replay_fn(**kwargs: Any) -> dict[str, Any]:
            ...
            for action in recovery_cut.selected_actions:
                ...
                if not success:
                    return {
                        "_error": "ACTION_UNSUPPORTED",
                        "action": getattr(action, "action_id", "unknown"),
                    }
            ...
            return {
                "spans": getattr(
                    result, "spans", result.get("spans", []) if isinstance(result, dict) else []
                ),
                "metrics": getattr(
                    result, "metrics", result.get("metrics", {}) if isinstance(result, dict) else {}
                ),
                "state_snapshot": getattr(
                    result,
                    "state_snapshot",
                    result.get("state_snapshot", None) if isinstance(result, dict) else None,
                ),
                "failure_status": getattr(
                    result,
                    "failure_status",
                    (
                        result.get("failure_status", "unresolved")
        ...
        metrics = {
            "regression_count": float(
                result.get("metrics", {}).get("regression_count", recovery_cut.regression_risk)
            ),
            "blast_radius": float(
                result.get("metrics", {}).get("blast_radius", recovery_cut.blast_radius)
            ),
        }
