...
class RecoveryAdapter(abc.ABC):
    ...
    @abc.abstractmethod
    def rollback(
        self, previous_state: dict[str, Any], target_state: dict[str, Any]
    ) -> dict[str, Any]:
...
class KubernetesRollbackAdapter(RecoveryAdapter):
    ...
    def apply(self, params: dict[str, Any]) -> dict[str, Any]:
        ...
        logger.info(
            f"Applying Kubernetes deployment update for {component_id} to version {target_version}"
        )
    ...
    def rollback(
        ...
        logger.info(
            f"Rolling back Kubernetes deployment {component_id} to revision {revert_version}"
        )
        # In a real system, we'd call: self.k8s_client.patch_namespaced_deployment_rollback(...)
        return {
            "component_id": component_id,
            "version_id": revert_version,
            "k8s_action": "rollback",
        }
...
class FeatureFlagAdapter(RecoveryAdapter):
    ...
    def rollback(
        ...
        if "tool_id" in previous_state:
            ...
            return {
                "tool_id": tool_id,
                "disabled": previous_state.get("disabled", False),
            ...
            return {
                ...
                "index_id": index,
                "ff_action": "switch_index_rollback",
            }

        raise ValueError(
            f"FeatureFlagAdapter could not determine rollback from state: {previous_state}"
        )
