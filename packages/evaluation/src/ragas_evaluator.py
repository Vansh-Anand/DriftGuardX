...
class RagasEvaluator:
    def __init__(self):
        self.enabled = HAS_RAGAS and bool(
            os.environ.get("OPENAI_API_KEY") or os.environ.get("LLM_API_KEY")
        )
        if not self.enabled:
            logger.warning(
                "Ragas evaluation is disabled. Missing 'ragas' package or 'LLM_API_KEY'."
            )

    def evaluate_episode(
        self, query: str, expected_answer: str, generated_answer: str, retrieved_contexts: list[str]
    ) -> dict[str, float]:
        if not self.enabled:
            ...
            return {
                "answer_correctness": (
                    1.0
                    if expected_answer and expected_answer.lower() in generated_answer.lower()
                    else 0.0
                ),
        ...
        try:
            result = evaluate(
                dataset, metrics=[answer_correctness, faithfulness, context_precision]
            )
            return {
