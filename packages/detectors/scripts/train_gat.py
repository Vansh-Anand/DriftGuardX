...
def run_ml_baselines(train_dataset, test_dataset):
    ...
    if len(np.unique(y_train)) > 1:
        for name, model in models.items():
            ...
            y_prob = model.predict_proba(X_test)[:, 1] if model.classes_.size > 1 else np.zeros_like(y_test)
        ...
        empty = {"roc_auc": 0, "pr_auc": 0, "f1": 0, "mcc": 0, "brier_score": 0, "hit@1": 0, "hit@3": 0, "hit@5": 0, "mrr": 0}
...
def train_main(args):
    ...
    results["centrality"] = {"roc_auc": 0, "pr_auc": 0, "f1": 0, "mcc": 0, "brier_score": 0, **cent_rnk}
