...
class TransferGuard:
    ...
    def _compute_provenance_similarity(
        self, source_env: ProvenanceEnvelope, target_env: ProvenanceEnvelope
    ) -> SimilarityResult:
        """
        ...
        """
        weights = {"model:": 3.0, "agent:": 2.5, "prompt:": 2.0, "tool:": 1.0, "data:": 1.0}
        ...
        if not source_nodes and not target_nodes:
            ...
            return SimilarityResult(
                score=score, matched_anchors=0, unrecognized_penalties=source_unrec + target_unrec
            )
    ...
    def can_transfer_diagnosis(
        ...
        if not self._verify_envelope(source_provenance) or not self._verify_envelope(
            target_provenance
        ):
            return False
        ...
        if (
            source_provenance.tenant_id == target_provenance.tenant_id
            and source_provenance.environment_hash == target_provenance.environment_hash
        ):
            return True
        ...
        calibration_shift = abs(
            source_provenance.calibration_evidence.empirical_risk
            - target_provenance.calibration_evidence.empirical_risk
        )
        return not calibration_shift > max_calibration_shift
