...
class LocalKMSProvider:
    ...
    def __init__(self):
        ...
        self._master_secret = os.environ.get("DGX_CAPABILITY_SECRET", "local_dev_fallback_secret_only").encode("utf-8")
        self._public_key = "pub_ed25519_" + hashlib.sha256(self._master_secret).hexdigest()[:16]
        
    def get_public_key(self) -> str:
        return self._public_key
        
    def sign_payload(self, payload_dict: dict[str, Any], signer_id: str = "system") -> CryptographicSignature:
        """
        ...
        signature_hash = hmac.new(self._master_secret, payload_str.encode("utf-8"), hashlib.sha256).digest()
    ...
    def verify_signature(self, payload_dict: dict[str, Any], signature: CryptographicSignature) -> bool:
        """
        Verifies the signature matches the payload.
        """
        payload_str = json.dumps(payload_dict, sort_keys=True, separators=(',', ':'))
        expected_hash = hmac.new(self._master_secret, payload_str.encode("utf-8"), hashlib.sha256).digest()
        expected_b64 = base64.b64encode(expected_hash).decode("utf-8")
        
        return (
            signature.signature == expected_b64 and
            signature.public_key == self._public_key
        )
