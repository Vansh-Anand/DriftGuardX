...
class CanaryTestFramework:
    ...
    async def execute_canary(
        ...
        manifest_orm = (await db.execute(select(ReplayStateManifestORM).where(ReplayStateManifestORM.run_id == uuid.UUID(original_run_id)))).scalar_one_or_none()
        run_orm = (await db.execute(select(RequestRunORM).where(RequestRunORM.id == uuid.UUID(original_run_id)))).scalar_one_or_none()
        trace_orm = (await db.execute(select(TraceArtifactORM).where(TraceArtifactORM.run_id == uuid.UUID(original_run_id)))).scalar_one_or_none()
        ...
        cost_incurred = ReplayCost(
            ...
            evidence_kind=episode.evidence_kind.value
        )
        print("EPISODE TYPE:", type(episode))
        print("IS SYNTHETIC:", getattr(episode, "is_synthetic", False))
        print("EVIDENCE KIND:", getattr(episode, "evidence_kind", None))
        
        if getattr(episode, "is_synthetic", False) or getattr(episode, "evidence_kind", None) == RecoveryEvidenceKind.SYNTHETIC_DEMO:
            candidate.causal_evidence.evidence_provenance = RecoveryEvidenceKind.SYNTHETIC_DEMO
            return BCRBStep(
                step_id=uuid.uuid4(),
                session_id=uuid.UUID(session_id),
                candidate_id=candidate.candidate_id,
        ...
        if "reliability" in episode.reliability_vector and "reliability" in run.reliability_vector:
            reliability_delta = episode.reliability_vector["reliability"] - run.reliability_vector["reliability"]
