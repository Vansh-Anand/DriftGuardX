...
class DeterministicRetriever(ComponentExecutor):
    def execute(self, inputs: dict[str, Any], *, version: ComponentVersion, seed: int = 42) -> dict[str, Any]:
        ...
        return {
            "documents": [
                {"id": f"doc-{seed}-1", "text": f"Found deterministic match 1 for: {query}", "score": 0.9},
                {"id": f"doc-{seed}-2", "text": f"Found deterministic match 2 for: {query}", "score": 0.8},
            ]
        }
...
class DeterministicTool(ComponentExecutor):
    def execute(self, inputs: dict[str, Any], *, version: ComponentVersion, seed: int = 42) -> dict[str, Any]:
        ...
        return {
            "tool_result": f"Tool {tool_name} executed deterministically with args {args}"
        }
