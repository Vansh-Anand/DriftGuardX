...
def generate_synthetic_episode(
    family_id: int, num_nodes: int = 5, fault_type: str = "fault_1", workload_type: str = "A"
    ...
    if workload_type == "A":
        ...
        for i in range(num_nodes):
            ...
            if i > 0:
                edge_type = (
                    ...
                    if types[(i - 1) % len(types)] == NodeType.AGENT
                    else EdgeType.DATA_DEPENDENCY
                )
                edges.append(
                    EdgeFeatures(
        ...
        edges.append(
            EdgeFeatures(
                ...
                edge_type=EdgeType.INTER_AGENT_COMMUNICATION,
                confidence=1.0,
                directionality=1.0,
            )
        )
...
def build_pyg_dataset(
    ...
    for i in range(num_episodes):
        ...
        _, _, _, data = generate_synthetic_episode(family_id=i, fault_type=fault, workload_type=wl)
        dataset.append(data)
