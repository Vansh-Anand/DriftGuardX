...
def generate_synthetic_episode(
    family_id: int, num_nodes: int = 5, fault_type: str = "retrieval_fault"
...
def build_pyg_dataset(num_episodes: int = 100) -> list[Data]:
    ...
    for i in range(num_episodes):
        ...
        diffusion_in, root_labels, symptom_labels = generate_synthetic_episode(
            family_id=i, fault_type=fault
        )

        x = torch.tensor(
            [[n.local_symptom_score, n.severity_weight] for n in diffusion_in.nodes],
            dtype=torch.float,
        )
