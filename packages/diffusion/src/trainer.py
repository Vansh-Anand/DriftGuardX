...
class DiffusionLoss(nn.Module):
    def __init__(
        self, lambda_symptom=0.5, lambda_sparse=0.01, lambda_contrastive=0.1, lambda_signature=1.0
    ):
        super().__init__()
    ...
    def forward(
        ...
        if contrastive_mask.any():
            loss_contrast = torch.mean(
                torch.clamp(
                    root_pred[contrastive_mask] - symptom_pred[contrastive_mask] + 0.5, min=0.0
                )
            )
        else:
            loss_contrast = 0.0
        ...
        total_loss = (
            loss_root
            + self.lambda_symptom * loss_symptom
            + self.lambda_sparse * loss_sparse
            + self.lambda_contrastive * loss_contrast
            + self.lambda_signature * loss_signature
        )
        ...
        return total_loss, {
            ...
            "contrast_loss": (
                loss_contrast.item() if isinstance(loss_contrast, torch.Tensor) else loss_contrast
            ),
            "signature_loss": (
                loss_signature.item()
                if isinstance(loss_signature, torch.Tensor)
                else loss_signature
            ),
        }
