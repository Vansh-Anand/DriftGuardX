...
from packages.diffusion.src.contracts import (
    DiffusionInput,
    DiffusionOutput,
...
class MultiAgentDiffusionEngine:
    ...
    def run_backward_diffusion(self, graph_input: DiffusionInput) -> GraphDiffusionResult:
            ...
            for target_id, incoming_edges in backward_edges.items():
                ...
                if target_score > 0:
                    for edge in incoming_edges:
                        ...
                        new_scores[edge.source_id] = min(
                            1.0, new_scores.get(edge.source_id, 0.0) + message
                        )
            scores = new_scores
        ...
        for node in graph_input.nodes:
            ...
            influential_edges = [
                edge.source_id
                for edge in backward_edges.get(node.node_id, [])
                if scores.get(edge.source_id, 0.0) > 0.1
            ]

            node_outputs[node.node_id] = DiffusionOutput(
                node_id=node.node_id,
                root_probability=min(
                    1.0, final_score
                ),  # Final accumulation determines root probability
