...
from packages.contracts.src.bcrb_models import AblationConfig, BCRBSession, StoppingCondition
...
async def run_single_ablation(
    tenant_id: str, config_name: str, ablation_config: AblationConfig, fault_scenario: FaultScenario
):
    ...
    invocations = [
        AgentInvocation(
            ...
            agent_identity=AgentIdentity(
                agent_type="retrieval", agent_id="ret-1", agent_version="1.0"
            ),
            start_time=datetime.now(),
            end_time=datetime.now(),
            metadata={
                "is_error": fault_scenario.fault_type
                in [FaultType.RETRIEVAL_FAILURE, FaultType.STALE_CORPUS]
            },
        ),
        AgentInvocation(
            run_id=run_uuid,
            ...
            agent_identity=AgentIdentity(
                agent_type="reasoning", agent_id="rea-1", agent_version="1.0"
            ),
            start_time=datetime.now(),
            end_time=datetime.now(),
    ...
    session = BCRBSession(
        run_id=run_uuid, tenant_id=tenant_uuid, budget_usd=1.0, total_spent_usd=0.0
    )
    ...
    return {
        ...
        "stopping_condition": (
            session.stopping_condition_met.value if session.stopping_condition_met else None
        ),
    }
...
async def main() -> None:
    ...
    for seed in range(42, 42 + args.seeds):
        ...
        for config_name, ablation_config in configs.items():
            ...
            try:
                ...
                print(
                    f"  -> Result: Success={res['success']}, Steps={res['steps_taken']}, Time={res['duration_seconds']:.2f}s"
                )
    ...
    if args.seeds == 1:
        ...
        for r in all_results:
            print(
                f"| {r['config']} | {r['success']} | {r['steps_taken']} | {r['duration_seconds']:.2f} | {r['total_spent_usd']:.4f} |"
            )
