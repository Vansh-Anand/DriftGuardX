...
def load_results(results_path: str) -> None:
...
def run_statistical_tests(bcrb_accs, baseline_accs):
    ...
    try:
        ...
        _w_stat, p_val_w = 0.0, 1.0  # zero differences

    return {"paired_t_pvalue": p_val_t, "wilcoxon_pvalue": p_val_w}
...
def evaluate(args) -> None:
    ...
    for name, accs in models.items():
        ...
        print(
            f"BCRB vs {name}: Paired t-test p={res['paired_t_pvalue']:.4e}, Wilcoxon p={res['wilcoxon_pvalue']:.4e}"
        )

    # Generate report
    report = {"trials": n_trials, "seed": args.seed, "models": {}}
    for name, accs in models.items():
        report["models"][name] = {
