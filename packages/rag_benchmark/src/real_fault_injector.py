...
from packages.rag_benchmark.src.fault_models import FaultScenario, RealControlledFaultInjector
...
class GenuineFaultInjector(RealControlledFaultInjector):
    ...
    def inject(self, pipeline: RealRAGPipeline, scenario: FaultScenario) -> None:
        ...
        if ft_val in ["INDEX_TOMBSTONE", "RETRIEVAL_FAILURE"]:
            ...
            self._restoration_state["orig_embed_dim"] = (
                getattr(pipeline.retriever, "embedding_dim", 768) if pipeline else 768
            )
            if pipeline:
                pipeline.retriever.embedding_dim = 1536
            ...
            if pipeline:
                pipeline.prompt_template = "Context: {query}\nProvide a single word answer: ERROR."
            ...
            if pipeline:
                pipeline.router = lambda x: "FALLBACK_ERROR_AGENT_ID"
    ...
    def reset(self, pipeline: RealRAGPipeline) -> None:
        ...
        if "orig_embed_dim" in self._restoration_state:
            if pipeline:
                pipeline.retriever.embedding_dim = self._restoration_state["orig_embed_dim"]
            os.environ["EMBEDDING_DIM"] = str(self._restoration_state["orig_embed_dim"])

        if "orig_prompt" in self._restoration_state:
            if pipeline:
                pipeline.prompt_template = self._restoration_state["orig_prompt"]
