...
class FaultInjector:
    ...
    def inject_retrieval_fault(self, corrupted_corpus: list[str]) -> None:
        """
        Simulates a corrupted index or retrieval failure by replacing the corpus.
        """
        self.pipeline.retriever.corpus = corrupted_corpus
        self.pipeline.retriever.embeddings = [
            self.pipeline.retriever._embed(t) for t in corrupted_corpus
        ]
        self.pipeline.version_tag = "v2-corrupted-index"

    def inject_prompt_fault(self, bad_prompt: str) -> None:
        """
    ...
    def inject_model_fault(self, new_model_name: str, high_temperature: float = 1.5) -> None:
        """
    ...
    def inject_policy_fault(self) -> None:
        """
        ...
        def loose_check(prompt: str, response: str, **kwargs) -> bool:
            ...
            if trace_ctx:
                ...
                builder.set_component(
                    ComponentType.POLICY_CHECK, uuid.UUID(int=3), "v2-loose-policy"
                )
    ...
    def reset(self, original_corpus: list[str]) -> None:
        """
        Resets the pipeline to normal state.
        """
        self.pipeline.retriever.corpus = original_corpus
        self.pipeline.retriever.embeddings = [
            self.pipeline.retriever._embed(t) for t in original_corpus
        ]
