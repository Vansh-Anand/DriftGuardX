...
from packages.evaluation.src.bandit_baselines import CandidateArm, RandomBudgetScheduler
...
def test_bcrb_uncertainty_spikes():
    ...
    scheduler.update("volatile", reward=1.0, cost=0.1)

    # pull 2: actual cost 0.9 (mean is now 0.5, but std_dev is high!)
    # remaining budget = 1.0 - 0.1 = 0.9
    assert scheduler.select_arm(arms) == "volatile"
    scheduler.update("volatile", reward=1.0, cost=0.9)
...
def test_bcrb_uncertainty_spikes_rejection():
    ...
    assert scheduler.select_arm(arms) == "volatile"
    scheduler.update("volatile", reward=1.0, cost=2.0)
...
def test_bcrb_underestimation():
    ...
    scheduler.update("liar", reward=1.0, cost=0.9)
