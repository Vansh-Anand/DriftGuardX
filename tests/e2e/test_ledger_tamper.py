...
from packages.ledger.src.chain import CertificateValidationError, LedgerChain
from packages.ledger.src.crypto import DevelopmentSigner
from packages.ledger.src.export import export_human_summary, export_machine_bundle
...
async def test_tamper_alter_previous_hash(temp_ledger, signer):
    ...
    async with aiosqlite.connect(temp_ledger.db_path) as db:
        ...
        await db.execute(
            "UPDATE ledger SET previous_hash = ?, payload = ? WHERE id = 2",
            ("FAKE_HASH", json.dumps(payload)),
        )
        await db.commit()
...
async def test_invalid_signature_on_append(temp_ledger, signer):
    ...
    with pytest.raises(
        CertificateValidationError, match="Cryptographic signature verification failed"
    ):
...
async def test_latency_scaling_benchmark(temp_ledger, signer, caplog):
    ...
    print(
        f"[BENCHMARK] Verified {N_CERTS} certs in {verify_latency:.3f}s (Avg {avg_verify:.2f}ms/cert)"
    )
