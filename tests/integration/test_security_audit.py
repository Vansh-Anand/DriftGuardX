...
async def setup_test_auth(db_session: AsyncSession):
    ...
    u_admin = UserORM(
        auth_subject=f"test-admin-sub-{u_admin_id}", email=f"admin-{u_admin_id}@test.com"
    )
    db_session.add(u_admin)
    u_viewer = UserORM(
        auth_subject=f"test-viewer-sub-{u_viewer_id}", email=f"viewer-{u_viewer_id}@test.com"
    )
    ...
    m_viewer = TenantMembershipORM(
        user_id=u_viewer.id, tenant_id=t_viewer.id, roles_json=["viewer"]
    )
    db_session.add(m_viewer)

    await db_session.commit()

    return {"t_admin": t_admin, "u_admin": u_admin, "t_viewer": t_viewer, "u_viewer": u_viewer}
...
async def test_authenticated_unauthorized_role(client: AsyncClient, monkeypatch, setup_test_auth):
    ...
    from packages.contracts.src.recovery_models import EvidenceClassification
    ...
    monkeypatch.setattr(
        "apps.api.src.services.recovery_pipeline.EndToEndRecoveryPipeline.execute_recovery_loop",
        mock_execute_recovery_loop,
    )

    response = await client.post(
        "/v1/recovery/trigger", json={"tenant_id": str(setup_test_auth["t_viewer"].id)}
    )
    print(response.json())
...
async def test_human_approval_workflow(
    client: AsyncClient, monkeypatch, setup_test_auth, db_session: AsyncSession
):
    client.headers["Authorization"] = "Bearer fake-token"
    ...
    response = await client.post(
        "/v1/recovery/trigger", json={"tenant_id": str(setup_test_auth["t_admin"].id)}
    )
    print(response.json())
    ...
    approve_res = await client.post(
        "/v1/recovery/approve", json={"approval_id": approval_id, "decision": "APPROVED"}
    )
    ...
    verify_res = await client.post("/v1/recovery/verify", json={"certificate": cert})

    assert verify_res.status_code == 200
    assert verify_res.json()["valid"] is True

    cert["certificate_hash"] = "tampered_hash_here"
    verify_res_bad = await client.post("/v1/recovery/verify", json={"certificate": cert})
