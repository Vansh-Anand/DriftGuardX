...
async def test_finalize_run_idempotency_and_validation(
    ...
    )  # matches make_tenant_id() in conftest.py
...
async def test_concurrent_finalize(client: AsyncClient, db_session: AsyncSession):
    ...
    res1, res2 = await asyncio.gather(call_finalize("COMPLETED"), call_finalize("FAILED"))
...
async def test_finalize_durability_failure(
