...
async def test_64_canary_invariants():
    ...
    with patch("packages.rag_pipeline.src.agents.AgentPipeline") as mock_pipeline:
        ...
        with patch("time.monotonic", side_effect=[0.0, 1.0]):  # 1 second = 1000ms latency
            ...
            result = await framework.async_validate_quarantine(
                rule, str(uuid.uuid4()), db=None, invariants=invariants
            )
...
async def test_65_automatic_rollback():
    ...
    with patch("packages.rag_pipeline.src.agents.AgentPipeline") as mock_pipeline:
        ...
        result = await framework.async_validate_quarantine(
            rule, str(uuid.uuid4()), db=mock_db, isolator=mock_isolator, invariants=invariants
        )
...
async def test_66_policy_gated_promotion():
    ...
    response = await approve_recovery(
        {
            "approval_id": str(uuid.uuid4()),
            "decision": "APPROVED",
            "simulate_promotion_policy": True,
        },
        mock_db,
        mock_user,
    )
...
async def test_67_worker_diagnosis_offload():
    ...
    with patch("arq.connections.create_pool", new_callable=AsyncMock) as mock_create_pool:
        ...
        with patch("apps.api.src.config.settings.environment", "dev"):
            response = await trigger_recovery(
                {"tenant_id": str(mock_user.tenant_id), "run_id": str(uuid.uuid4())},
                mock_db,
                mock_user,
            )
