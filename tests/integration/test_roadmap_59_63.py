...
from packages.contracts.src.models import RecoveryEvidenceKind
...
async def test_61_durable_quarantine(db_session):
    ...
    rule = await isolator.async_apply_quarantine(
    ...
    result = await db_session.execute(select(QuarantineRuleORM).where(QuarantineRuleORM.tenant_id == uuid.UUID(tenant_id)))
...
def test_62_fallback_routing():
    ...
    state = pipeline.run("test query", str(uuid.uuid4()), str(uuid.uuid4()), quarantined_agents=quarantined)
...
async def test_59_safety_gate():
    ...
    with patch('packages.bcrb.src.orchestrator.BCRBOrchestrator') as mock_bcrb:
        ...
        invocations = [
            AgentInvocation(
                invocation_id=uuid.uuid4(), run_id=uuid.uuid4(), tenant_id=uuid.uuid4(),
                agent_name="retrieval", start_time=datetime.now(UTC), end_time=datetime.now(UTC),
            )
        ]
        
        with pytest.raises(RuntimeError, match="Safety violation: cannot automatically repair from synthetic evidence"):
