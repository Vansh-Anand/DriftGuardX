...
def test_certificate_integrity_tampering():
    ...
    payload_to_sign = {
        ...
        "evidence_class": "COUNTERFACTUAL_REPLAY",
        "hash": "validhash123",
    }
    ...
    # Verify original
    assert kms_provider.verify_signature(payload_to_sign, sig) is True
    ...
    # Verify tampered
    assert kms_provider.verify_signature(tampered_payload, sig) is False

    # Tamper run_id
    tampered_payload2 = payload_to_sign.copy()
    tampered_payload2["run_id"] = str(uuid.uuid4())
    assert kms_provider.verify_signature(tampered_payload2, sig) is False
...
def test_hash_chain_integrity():
    ...
    # 2. Trace
    trace = TraceArtifact(
        tenant_id=tenant_id, run_id=run.id, pipeline_id=pipeline_id, spans=[], run_hash=run.run_hash
    )
    trace.trace_hash = trace.compute_hash()

    # 3. Diagnosis
    diagnosis = Diagnosis(
        tenant_id=tenant_id, run_id=run.id, claims=[], trace_hash=trace.trace_hash
    )
    ...
    step = BCRBStep(
        session_id=uuid.uuid4(), candidate_id=candidate.candidate_id, replay_hash=replay.replay_hash
    )
    ...
    intervention = Intervention(
        ...
        from_version_tag="v2",
        to_version_tag="v1",
        posterior_hash=step.posterior_hash,
    )
    intervention.intervention_hash = intervention.compute_hash()

    # 8. Report
    report = RootCauseReport(
        tenant_id=tenant_id, run_id=run.id, recovery_hash=intervention.intervention_hash
    )
...
def test_tamper_detection_breaks_chain():
    ...
    trace.evidence_class = EvidenceClassification.SYNTHETIC_SIMULATION
