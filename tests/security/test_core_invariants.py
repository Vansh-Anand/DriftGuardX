...
def test_recovery_started_implies_confirmed():
    ...
    class _Adapter:
        ...
        def current_beliefs(self):
            return dict(self.m.beliefs)

        def entropy(self):
            return self.m.entropy()

    stop, outcome, _ = stopping_rule.is_sufficient(
        None, resource_context, _Adapter(belief), [{"candidate_id": "A"}]
    )
...
def test_tampered_ree_fails_verification():
    ...
    ree = ReplayEquivalenceEnvelope(
        trace_id="t1",
        recovery_cut=cut,
        invariants=[],
        snapshot_hash="shash1",
        intervened_variables=[],
        allowed_causal_descendants=[],
        exogenous_variables={},
    )
    ...
    h1 = hashlib.sha256(
        json.dumps(ree.model_dump(mode="json"), sort_keys=True).encode()
    ).hexdigest()
    ...
    ctx = AccessContext(
        ...
        authenticated_roles=[Role.VIEWER.value],
        capabilities=[cap],
        expires_at=datetime.now(UTC) + timedelta(hours=1),
    )
...
def test_transport_descriptor_tampering():
    desc = CausalEnvironmentDescriptor(
        environment_id="e1",
        timestamp=datetime.now(UTC),
        package_versions={"a": "1"},
        schema_definitions={},
        infrastructure_fingerprint="f1",
        signature="sig",
        tenant_id=str(uuid.uuid4()),
        model="m1",
        prompt="p1",
        ...
        policy="pol1",
        index="idx1",
        data_distribution_fingerprint="d1",
        execution_configuration={},
        causal_graph_hash="c1",
        provenance_hash="p1",
    )
    ...
    h1 = hashlib.sha256(
        json.dumps(desc.model_dump(exclude={"signature"}, mode="json"), sort_keys=True).encode()
    ).hexdigest()
    h2 = hashlib.sha256(json.dumps(data, sort_keys=True).encode()).hexdigest()
    assert h1 != h2
...
def test_transport_decision_evidence_tampering():
    ...
    dec = TransportabilityDecision(
        recovery_id="r1",
        source_environment="s1",
        target_environment="t1",
        ...
        policy_version="1",
        footprint_hash="h1",
        source_descriptor_signature="s_h",
        target_descriptor_signature="t_h",
...
def test_no_auto_recovery_from_unresolved():
    ...
    sources = SourceSelector.select_sources(
        {"A": 0.4}, StoppingOutcome.UNRESOLVED, SourceSelectionPolicy.CREDIBLE_SET
    )
...
def test_minimum_recovery_cut_blocks_all_paths():
    ...
    nodes = [
        ...
        GraphNode(id="out", type=NodeType.REQUEST, label="out"),
    ]
    edges = [
        GraphEdge(id="A->X", source="A", target="X", type=EdgeType.DATA_DEPENDENCY),
        GraphEdge(id="X->out", source="X", target="out", type=EdgeType.DATA_DEPENDENCY),
    ]
    graph = CausalGraph(
        tenant_id=uuid.uuid4(), run_id=uuid.uuid4(), trace_digest="d", nodes=nodes, edges=edges
    )
