...
async def test_concurrency_stress(diagnosis_engine):
    ...
    for level in concurrency_levels:
        ...
        print(
            f"Concurrency: {level:3d} | Success: {success_count}/{level} | Time: {duration:.2f}s | P50: {p50*1000:.1f}ms | P95: {p95*1000:.1f}ms | P99: {p99*1000:.1f}ms"
        )
...
async def test_trace_size_stress(orchestrator):
    ...
    for size in trace_sizes:
        ...
        print(
            f"Spans: {size:6d} | BCRB Candidates: {len(candidates)} | Processing Time: {duration:.3f}s"
        )
