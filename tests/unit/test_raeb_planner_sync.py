...
def test_A_B_ReplayEpisode_maps_exactly():
    ...
    replay_m = ReplayEpisode(
        tenant_id=uuid.uuid4(),
        run_id=trace.run_id,
        swapped_component_type=ComponentType.GENERATOR,  # using Generator as model
        original_version_id=uuid.uuid4(),
        replay_version_id=uuid.uuid4(),
        original_version_tag="v1",
        replay_version_tag="v2",
...
def test_J_heuristic_estimator_metadata():
    ...
    eval_res = gateway.evaluate_admissibility(
        trace, replay, belief_model=belief, trusted_timestamp=ts
    )
