...
def create_mock_trace(run):
    span1 = SpanRecord(
        ...
        name="root",
        start_time=datetime.datetime(2026, 9, 1, tzinfo=datetime.timezone.utc),
...
def test_replay_engine_is_strictly_deterministic():
    ...
    spec = InterventionSpec(
        target_component=ComponentType.RETRIEVER, current_version="v1", candidate_version="v2-exp",
        intervention_type="alternate_stable",
    )
    
    registry = VersionRegistry()
    registry.register(ComponentVersion(id=trace.spans[1].component_version_id, component_type=ComponentType.RETRIEVER, version_tag="v1", config_hash="h1"))
    
    cv_v2 = ComponentVersion(id=uuid.uuid4(), component_type=ComponentType.RETRIEVER, version_tag="v2-exp", config_hash="h2")
