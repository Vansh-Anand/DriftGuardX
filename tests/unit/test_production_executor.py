...
from packages.recovery.src.actions import RecoveryProposal, RecoveryActionType, ExecutionMode
...
def test_production_executor_apply():
    ...
    proposal = RecoveryProposal(
        ...
        params={"component_id": "api", "target_version_id": "v2", "expected_current_version_id": "v1"},
    ...
    assert res_ff.side_effects == {"tool_id": "calculator", "disabled": True, "ff_action": "disable_tool"}
...
def test_production_executor_rollback():
    ...
    proposal = RecoveryProposal(
        action_type=RecoveryActionType.ROLLBACK_COMPONENT,
        params={"component_id": "worker", "target_version_id": "v2", "expected_current_version_id": "current_v1"},
        execution_mode=ExecutionMode.APPROVED,
        tenant_id="t1",
        node_id="node1",
        run_id=uuid.uuid4(),
    ...
    assert comp_res.side_effects == {"component_id": "worker", "version_id": "current_v1", "k8s_action": "rollback"}
...
def test_feature_flag_rollback():
    ...
    comp_res = executor.compensate(capsule)
    assert comp_res.success
    assert comp_res.side_effects == {"tool_id": "calculator", "disabled": False, "ff_action": "enable_tool"}
