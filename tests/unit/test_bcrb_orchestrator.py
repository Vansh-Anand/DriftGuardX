...
async def test_sequential_execution_and_budget_exhaustion():
    ...
    session = BCRBSession(run_id=uuid.UUID(run_id), tenant_id=uuid.UUID(tenant_id), budget_usd=0.06)
...
async def test_no_fake_posterior_update():
    ...
    session = BCRBSession(run_id=uuid.UUID(run_id), tenant_id=uuid.UUID(tenant_id), budget_usd=10.0)
    ...
    tested_cand = next(
        c
        for c in result_session.candidates
        if c.candidate_id == result_session.steps[-1].candidate_id
    )
...
def test_diagnosis_engine_unknown():
    ...
    step = BCRBStep(
        session_id=uuid.uuid4(), candidate_id=cand.candidate_id, status=BCRBStepStatus.COMPLETED
    )
