...
def test_verifier_adversarial_normalization(attack_payload, forbidden_word):
    ...
    assert (
        DeterministicVerifier.verify_no_forbidden_words(attack_payload, [forbidden_word]) is False
    )
...
def test_verifier_multiple_words():
    ...
    assert (
        DeterministicVerifier.verify_no_forbidden_words("I want to steal data", ["steal", "hack"])
        is False
    )
