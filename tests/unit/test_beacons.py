...
def test_cosine_drift_comparator():
    ...
    decision_boundary = comparator_boundary.compare(
        "this is a test", "completely unrelated semantic payload"
    )
    ...
    decision_above = comparator_above.compare(
        "this is a test", "completely unrelated semantic payload"
    )
...
def test_drift_beacon_semantic_drift():
    ...
    baseline_outputs = {"probe_1": "The quick brown fox jumps over the lazy dog."}

    beacon = DriftBeacon("provider_1", baseline_outputs)

    # Runner returning exactly the same text
    result = beacon.run_probe(
        "probe_1", None, lambda x: {"text": "The quick brown fox jumps over the lazy dog."}
    )
    ...
    result_drift = beacon.run_probe(
        "probe_1", None, lambda x: {"text": "Cataclysmic error: model failed."}
    )
