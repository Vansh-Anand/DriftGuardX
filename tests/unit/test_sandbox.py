...
def test_read_outside_root(sandbox_dir):
    ...
    with pytest.raises(RuntimeError, match="Sandbox error.*blocked"):
        SandboxedWorker.run(exploit_read_outside, {"outside_file": outside_file}, enable_arc=False, sandbox_work_dir=sandbox_dir)
...
def test_sibling_path_prefix_traversal(sandbox_dir):
    ...
    with pytest.raises(RuntimeError, match="Sandbox error.*blocked"):
        SandboxedWorker.run(exploit_read_sibling, {"target": target}, enable_arc=False, sandbox_work_dir=sandbox_dir)
...
def test_symlink_redirection_escape(sandbox_dir):
    ...
    with pytest.raises(RuntimeError, match="Sandbox error.*blocked"):
        SandboxedWorker.run(exploit_read_symlink, {"symlink_path": symlink_path}, enable_arc=False, sandbox_work_dir=sandbox_dir)
...
def test_naive_fixture_naming_bypass(sandbox_dir):
    ...
    with pytest.raises(RuntimeError, match="Sandbox error.*blocked"):
        SandboxedWorker.run(exploit_write_naive, {"outside_target": outside_target}, enable_arc=False, sandbox_work_dir=sandbox_dir)
...
def test_write_outside_temp_root(sandbox_dir):
    with pytest.raises(RuntimeError, match="Sandbox error.*blocked"):
        SandboxedWorker.run(exploit_write_outside, {"sandbox_dir": sandbox_dir}, enable_arc=False, sandbox_work_dir=sandbox_dir)
...
def test_timeout_resistant_child(sandbox_dir):
    with pytest.raises(TimeoutError, match="Sandboxed execution timed out"):
        SandboxedWorker.run(exploit_timeout, {}, timeout_seconds=1, enable_arc=False, sandbox_work_dir=sandbox_dir)
...
def test_large_output_bound(sandbox_dir):
    with pytest.raises(RuntimeError, match="Sandbox error.*exceeds sandbox bound"):
        SandboxedWorker.run(exploit_large_output, {}, enable_arc=False, sandbox_work_dir=sandbox_dir)
