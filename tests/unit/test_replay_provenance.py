...
async def test_synthetic_executor_yields_synthetic_demo_provenance():
    ...
    original_run = RequestRun(
        id=run_id, tenant_id=tenant_id, pipeline_id=uuid.uuid4(), evidence_class="PRODUCTION"
    )
    ...
    assert episode.evidence_class == EvidenceClassification.SYNTHETIC_SIMULATION
...
async def test_bcrb_rejects_synthetic_evidence(monkeypatch):
    ...
    mock_run = MockORM(
        ...
        error_message=None,
        reliability_vector={},
        evidence_class="REAL_PRODUCTION",
    )
    ...
    monkeypatch.setattr(
        "packages.replay.src.engine.ReplayEngine.execute_replay",
        lambda *args, **kwargs: (MockEpisode(), None),
    )

    # Execute
    step = await framework.execute_canary(
        candidate=candidate, original_run_id=str(run_id), session_id=str(uuid.uuid4()), db=MockDB()
    )
    ...
    assert (
        candidate.causal_evidence.evidence_provenance == EvidenceClassification.SYNTHETIC_SIMULATION
    )
