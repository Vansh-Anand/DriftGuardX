...
from packages.rag_pipeline.src.providers import (
...
async def test_local_deterministic_provider():
    ...
    response = await provider.generate(
        prompt="Hello", context=["World"], temperature=0.5, max_tokens=100
    )
...
def test_invoke_llm_real_mode_no_key():
    with patch.dict(os.environ, {"DGX_REAL_MODE": "1", "OPENAI_API_KEY": ""}):
        with pytest.raises(
            RuntimeError, match="real_mode is enabled but OPENAI_API_KEY is not set."
        ):
            invoke_llm("prompt", "{}")
