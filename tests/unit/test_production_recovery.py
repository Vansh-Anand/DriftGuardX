...
from packages.recovery.src.adapters import FeatureFlagAdapter, KubernetesRollbackAdapter
...
def test_feature_flag_adapter_apply_and_rollback():
    ...
    assert se == {
        "component_id": "reasoning_agent",
        "model": "gpt-4-stable",
        "ff_action": "route_model",
    }
    ...
    assert rb == {
        "component_id": "reasoning_agent",
        "model": "gpt-4-old",
        "ff_action": "route_model_rollback",
    }

    # Switch index
    se = adapter.apply({"component_id": "retrieval_agent", "target_index_id": "index_v2"})
    assert se == {
        "component_id": "retrieval_agent",
        "index_id": "index_v2",
        "ff_action": "switch_index",
    }
