...
def make_env(components, risk=0.1, env_hash="abc"):
    return ProvenanceEnvelope(
        tenant_id="test",
        components=components,
        environment_hash=env_hash,
        calibration_evidence=CalibrationEvidence(confidence_interval=0.05, support_size=100, empirical_risk=risk)
    )
...
def test_can_transfer_diagnosis():
    ...
    assert guard.can_transfer_diagnosis(
        source_prov, target_prov, max_calibration_shift=0.05
    ) is True
    ...
    assert guard.can_transfer_diagnosis(
        source_prov, target_prov_shift, max_calibration_shift=0.05
    ) is False
