...
class EndToEndRecoveryPipeline:
    ...
    async def execute_recovery_loop(
        ...
        session = await orchestrator.execute_session(
            session, list(invocations), failure_symptom, db
        )
        ...
        if diagnosis.status == "INSUFFICIENT_EVIDENCE":
            print("Diagnosis concluded with INSUFFICIENT_EVIDENCE")
            return type(
                ...
                (object,),
                {
                    ...
                    "highest_posterior": diagnosis.highest_posterior,
                    "threshold": diagnosis.confidence_threshold,
                    "highest_candidate_component": diagnosis.highest_candidate_component,
                },
            )
        ...
        try:
            ...
            success = await self.canary_framework.async_validate_quarantine(
                rule, run_id, db, isolator=self.isolator, invariants=invariants
            )
            if not success:
        ...
        if (
            getattr(self, "_force_automated", False)
            and evidence_kind == EvidenceClassification.SYNTHETIC_SIMULATION
        ):
            raise RuntimeError(
                "Safety violation: cannot automatically repair from synthetic evidence"
            )
