...
class BenchmarkHarness:
    ...
    def generate_scenarios(self) -> list[dict]:
        return [
            {
                "id": "retriever_regression",
                "targets": [
                    FailureTarget(
                        node_id="retriever", failure_type="hallucination", severity="high"
                    )
                ],
            },
            {
                ...
                "targets": [
                    FailureTarget(node_id="prompt", failure_type="schema_error", severity="medium")
                ],
            },
            {
                "id": "memory_poisoning",
                "targets": [
                    FailureTarget(node_id="memory", failure_type="context_limit", severity="low")
                ],
            },
            {
                "id": "tool_output_corruption",
                "targets": [
                    FailureTarget(node_id="tool", failure_type="corruption", severity="high")
                ],
            },
            {
                "id": "tool_schema_change",
                "targets": [
                    FailureTarget(node_id="tool", failure_type="schema_error", severity="high")
                ],
            },
            {
                "id": "model_drift",
            ...
            {
                "id": "external_api_state_change",
                "targets": [
                    FailureTarget(node_id="api", failure_type="unavailable", severity="critical")
                ],
            },
            {
                "id": "policy_mismatch",
                "targets": [
                    FailureTarget(node_id="policy", failure_type="unauthorized", severity="high")
                ],
            },
            {
                "id": "cross_env_diff",
                "targets": [FailureTarget(node_id="env", failure_type="mismatch", severity="high")],
            },
            {
                "id": "multi_cause_failure",
                "targets": [
                    FailureTarget(node_id="model", failure_type="hallucination", severity="high"),
                    FailureTarget(
                        node_id="retriever", failure_type="hallucination", severity="high"
    ...
    def run_all(self):
        ...
        for profile in profiles:
            ...
            for scenario in scenarios:
                ...
                tokens_list = []
                success_count = 0
                for _i in range(self.num_trials):
                ...
                profile_results.append(
                    {
                        ...
                        "std_time_ms": (
                            statistics.stdev(scenario_times) if self.num_trials > 1 else 0.0
                        ),
