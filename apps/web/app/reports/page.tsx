    ...
    <PageLayout title="Diagnosis Reports" subtitle="Root cause analysis with Hoeffding statistical certification" badge={badge}>
      <div className="p-8">
        <div className="border border-[#0a0a0a]">
          <div className="grid grid-cols-[140px_1fr_120px_120px_80px_120px] border-b border-[#0a0a0a] bg-[#0a0a0a] text-[#ECEAE2]">
            {['Run ID', 'Recommended Action', 'Status', 'Confidence', 'Trials', ''].map(h => (
              <div key={h} className="font-mono text-[10px] tracking-[0.1em] uppercase px-4 py-3">{h}</div>
            ))}
          </div>
          {REPORTS.map((r, i) => (
            <div key={r.run_id} className={`grid grid-cols-[140px_1fr_120px_120px_80px_120px] items-center ${i > 0 ? 'border-t border-[#0a0a0a]/10' : ''} hover:bg-[#0a0a0a]/5 transition-colors`}>
